...
func (repo *Repository) Exist(out interface{}, where ...interface{}) (e error) {
	...
	notFound := sql.RecordNotFound()
	if notFound {
		return ErrRecordNotFound
	} else if sql.Error != nil {
		return sql.Error
	} else {
		return nil
	}
...
// ModifyFunc 使用函数更新
func (repo *Repository) ModifyFunc(val interface{}, modifier func(interface{}), where ...interface{}) (affects int64, e error) {
	defer func() {
		if err := recover(); err != nil {
			e = fmt.Errorf("%v", err)
		}
	...
	if where != nil {
		db := repo.DB.Model(val).Where(where[0], where[1:]...).Updates(i)
		return db.RowsAffected, db.Error
	}
	db := repo.DB.Model(val).Updates(i)
	return db.RowsAffected, db.Error
}
